...
float VecDot(float v1[3], float v2[3])
{
  return (v1[0]*v2[0]+v1[1]*v2[1]+v1[2]*v2[2]);
}

int VecCross(float v1[3], float v2[3], float v[3])
{
...
int StoreVelocity(t_trxframe fr, t_topology top,
                  MMatrix_t posOW, atom_id *OWIndex, int size, 
		  int nWt, int start, bool bMW)
{
  int i, j, k, ll, mm, nn;
  ...
  if(bMW==FALSE)
  {
    for(i=0;i<nWt;i++)
    {
      for(k=0; k<DIM; k++)
      {
	posOW->m[i+start][k] = fr.v[OWIndex[i]][k];
      }
    }
    return 0;
  }
...
//  printf("per=%d, size=%d, nWt=%d\n", per, size, nWt);
  for(i=0; i<nWt; i++)
  {
    ...
    for(j=0, mass=0.0; j<per; j++)
    {
      ...
      for(k=0; k<DIM; k++)
      {
	posOW->m[i+start][k] += fr.v[OWIndex[ll]][k]*top.atoms.atom[OWIndex[ll]].m;
      }
    }
    for(k=0; k<DIM; k++)
    {
...
float ZRemovePBC(float z, float Lz)
{
  int i, j, k;
