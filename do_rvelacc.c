#include "do_rvelacc.h"

void do_rvelacc(char *fnTPS, char *fnTRX, char *fnRDF, 
            int ng, int *isize, char **grpname, atom_id **index,
	    int snp, real dT, real chunk, bool bMW, float xc, float yc, float r1, float r2, int nskipt0)
{
  int i, j, k;
 ...
 **/
  ...
  for(hsection=0;hsection<totalSection*2;hsection++)
  {
    ...
    if(hsection)
    {
      RVELACCComputing(posOW, velOW, nWt, &nhits, msdWt, msdWt0, msdWtTemp,
		   origins, nmax, nskipt0, xc, yc, r1, r2, FALSE, L);
    }
...
int RVELACCComputing(MMatrix_t Wt, MMatrix_t Wtv, int nWt, float *nhits, MMatrix_t msdWt, MMatrix_t msdWt0, MMatrix_t msdWtTemp,
                 int origins,int nmax, int nskipt0,
                 float xc, float yc, float r1, float r2,
		 bool bRDF, real L[3])
{
  ...
  for(j=0;j<origins;j=j+nskipt0)
  {
	...
	rdis2 = (Wt->m[kend][0]-xc)*(Wt->m[kend][0]-xc) + (Wt->m[kend][1]-yc)*(Wt->m[kend][1]-yc);
	...
	VecCross(unitvecz, unitvecx, unitvecy);
