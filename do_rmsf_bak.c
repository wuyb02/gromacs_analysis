 ...
 **/
  ...
  MMatrix_t rmsf = CreateMatrix(totalSection,nOS);
...
//    printf("Section %d of total section %d loaded\n",section,totalSection);
    analyze_RMSF(posOS, densOS, mr, mr2, nOS, L);
...
int analyze_RMSF(MMatrix_t posOS, MMatrix_t densOS, MMatrix_t mr, MMatrix_t mr2, 
                 int nOS, float L[3])
{
  int i,j,k,me,meAng,meAccept,m;
  real pos[3], mpos[3], diff[3];
  ...
  for(i=0; i<nOS; i++)
  {
    ...
    if(densOS->m[i][0]>1e-4)
    {
      for(k=0; k<3; k++)
      {
	mpos[k]=mr->m[i][k]/densOS->m[i][0];
	diff[k]=periodicity(pos[k]-mpos[k], L[k]);
	pos[k]=mpos[k]+diff[k];
      }
    }
...
int Output(MMatrix_t densOS, MMatrix_t mr, MMatrix_t mr2, MMatrix_t rmsf, int nOS, int totalSection, char *fn)
{
  FILE *fp;
  int i,j,k;
  float frmsf;
