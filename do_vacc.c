...
void do_vacc(char *fnTPS, char *fnTRX, char *fnRDF, 
 ...
 **/
  ...
  MMatrix_t grids = CreateMatrix(Ln[0]*Ln[1]*Ln[2],3);
  ...
  for(i=0; i<Ln[0]; i++)
  {
    for(j=0; j<Ln[1]; j++)
    {
      for(k=0; k<Ln[2]; k++)
      {
	ll=i*Ln[1]*Ln[2]+j*Ln[2]+k;
	grids->m[ll][0]=(i+0.5)*gridsize;
	grids->m[ll][1]=(j+0.5)*gridsize;
	grids->m[ll][2]=(k+0.5)*gridsize;
//	printf("%10.3f%10.3f%10.3f\n", grids->m[ll][0], grids->m[ll][1], grids->m[ll][2]);
      }
    }
  }
 ...
 **/
  for(section=0;section<totalSection;section++)
  {
    ...
    analyze_VACC(posOS, posOW, grids, vacc, fvacc, top, isize[emoPEG], index[emoPEG], grpname[emoPEG],
                 nOS, solutesize, section, L, Ln);
  }
...
//    printf("%10s%10.3f\n", *(top.atoms.atomtype[index[iOS]]), sigma);
    ...
    printf("Finish%7dof %7d,%6.3f,%6.3f,%6.3f, From index[%5d]%5d%5s,%6.3f,%6.3f,%6.3f,%6.3f,result is %3d\n", 
	   i, Ln[0]*Ln[1]*Ln[2], 
	   grids->m[i][0], grids->m[i][1], grids->m[i][2],
